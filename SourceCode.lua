--[[
Skids Source Code Leaked By nexus#6613~ UwU
]]
...
local SaveManager = {} do
    ...
    function SaveManager:Save(name)
        local fullPath = 'SkidsSourceLeaked/configs/' .. name .. '.json'
    ...
    function SaveManager:Load(name)
        local file = 'SkidsSourceLeaked/configs/' .. name .. '.json'
    ...
    function SaveManager.Refresh()
        local list = listfiles('SkidsSourceLeaked/configs')
...
local Window = UI:CreateWindow({
    Title = string.format('Skidded Funky Friday Autoplayer | Source leaked by nexus', metadata.version, metadata.updated),
    AutoShow = true,
...
if type(readfile) == 'function' and type(writefile) == 'function' and type(makefolder) == 'function' and type(isfolder) == 'function' then
    makefolder('SkidsSourceLeaked')
    makefolder('SkidsSourceLeaked\\configs')
...
do
    ...
    themeManager:SetFolder('SkidsSourceLeaked')
...
UI:Notify(string.format('source leaked by nexus~ UwU', tick() - start), 5)
Library:SetWatermarkVisibility(true)
Library:SetWatermark('Source Code leaked by nexus#6613')
